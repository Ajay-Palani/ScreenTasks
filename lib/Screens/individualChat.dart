...
import 'package:task5/Bloc/messageBloc.dart';
...
class _IndividualchatState extends State<Individualchat> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => MessageBloc()..add(SendMessageEvent(this.message)),

      child: BlocListener<MessageBloc, MessageState>(
        listener: (context, state) {
          if(state is SendMessageErrorState){
            showDialog(context: context, builder: (context) {
              return AlertDialog(title: Text(state.error),);
            },);
          }
        },
        ...
        child: Scaffold(
          ...
          body: BlocBuilder<MessageBloc, MessageState>(builder: (context, state) {
            if(state is MessageScreenLoadingState){
              return Shimmer.fromColors(child: Container(height: 20, width: 40,decoration: BoxDecoration(color: Colors.red),), baseColor: Colors.red, highlightColor: Colors.grey);
            }
            else if(state is SendMessageSuccessState){
              return sendReceiveMessages(state.isMessageEnter, state.message);
            }
            else{
  ...
  Widget sendReceiveMessages(isMessageEnter, message){
    return Column(
      children: [
        Expanded(
